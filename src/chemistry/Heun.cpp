...
//========================================================================================
//! \file Heun.cpp
//! \brief implementation of the Heun method solver (improved Euler with 2 order accuracy)
    ...
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        for (int ispec=0; ispec<=NSPECIES; ispec++) {
          y[ispec] = y0[ispec] = y1[ispec] = pmy_spec_->s(ispec,k,j,i)/u(IDN,k,j,i);
        }
        ...
        while (tnow < tend) {
          ...
          tsub = cfl_cool_sub0 * GetChemTime(y, ydot0, E, Edot0);
          ...
          //calculate reaction rates
          pmy_spec_->chemnet.RHS(time + tsub, y1, E1, ydot1);
          //calculate heating and cooling rats
          if (NON_BAROTROPIC_EOS) {
            Edot1 = pmy_spec_->chemnet.Edot(time + tsub, y1, E1);
          }
